import { getPolymarketMarkets, getPolymarketTrending } from './polymarket.service';
import { getKalshiMarkets, getKalshiTrending } from './kalshi.service';
...
export function transformPolymarketMarket(market: any): Market {
    ...
    if (Math.random() < 0.05) { // Log 5% of markets to avoid spam
        console.log('Raw Polymarket data sample:', JSON.stringify({
            id: market.id,
    ...
    if (market.outcomePrices) {
        // Handle both array of strings and JSON string format
...
export function transformKalshiMarket(market: any): Market {
    // Debug logging for Kalshi
    if (Math.random() < 0.05) {
        console.log('Raw Kalshi data sample:', JSON.stringify({
...
export function transformPolymarketEvent(event: any): Market {
    // Pick the most active market or the first one
